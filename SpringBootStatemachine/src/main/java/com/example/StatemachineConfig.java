...
public class StatemachineConfig extends EnumStateMachineConfigurerAdapter<OrderState, OrderEvent> {
    ...
    public void configure(StateMachineTransitionConfigurer<OrderState, OrderEvent> transitions)
        ...
        transitions
                .withExternal()
                    // 접수 -> (주문 신청) -> 주문 대기
                    .source(OrderState.RECEIPTING).target(OrderState.ORDER_PENDING).event(OrderEvent.OFFER_RECEIPT)
                    .and()
                .withExternal()
                    // 주문 대기 -> (구매) -> 배송 대기
                    .source(OrderState.ORDER_PENDING).target(OrderState.DELIVERY_PENDING).event(OrderEvent.PURCHASE)
                    .and()
                .withExternal()
                    // 배송 대기 -> (인계) -> 배송 중
                    .source(OrderState.DELIVERY_PENDING).target(OrderState.DELIVERY_PROCEEDING).event(OrderEvent.TAKEOVER)
                    .and()
                .withExternal()
                    // 배송 중 -> (확정) -> 완료
                    .source(OrderState.DELIVERY_PROCEEDING).target(OrderState.COMPLETED).event(OrderEvent.CONFIRM);
    }
